...
# Создание батчей
bv = artm.BatchVectorizer(data_path='vw.txt', data_format='vowpal_wabbit', batch_size=3000, target_folder='PLSA_batches')
bv2 = artm.BatchVectorizer(data_path='vw2.txt', data_format='vowpal_wabbit', batch_size=3000, target_folder='PLSA_batches2')
bv3 = artm.BatchVectorizer(data_path='vw3.txt', data_format='vowpal_wabbit', batch_size=3000, target_folder='PLSA_batches3')

# Функция создания и обучения модели
def make_and_train_PLSA(num_topics: list[int], num_collection_passes: list[int], num_doc_passes: list[int]):
  global results
    ...
    for param2 in num_collection_passes:
      for param3 in num_doc_passes:
        for param4 in range(1, 3+1):
          ...
          results.loc[ len(results.index) ] = [ 'PLSA', param1, param2, param3, '{0}-gramm'.format(param4),
